...
public class TableLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        if (state.didStructureChange()) {
            remeasureForce(recycler, state, getWidth(), getHeight());
        }
    ...
    public void onMeasure(@NonNull RecyclerView.Recycler recycler, @NonNull RecyclerView.State state, int widthSpec, int heightSpec) {
        ...
        int w2 = View.MeasureSpec.getSize(widthSpec), h2 = View.MeasureSpec.getSize(heightSpec);
        l("On Measure: " + w1 +"=>"+w2+", "+h1+"=>"+h2);
        remeasureIfNeeded(recycler, state, View.MeasureSpec.getSize(widthSpec), View.MeasureSpec.getSize(heightSpec));
    }

    private void remeasureForce(RecyclerView.Recycler recycler, RecyclerView.State state, int width, int height) {
        widthRef = width;
        heightRef = height;
        remeasureTable(recycler, state);
    }

    private void remeasureIfNeeded(RecyclerView.Recycler recycler, RecyclerView.State state, int width, int height) {
        if (width == widthRef && height == heightRef) return;
        remeasureForce(recycler, state, width, height);
    }
...
//=========================================================================================================================
    ...
    private void remeasureTable(RecyclerView.Recycler recycler, RecyclerView.State state) {
        l("Remeasuring table");
        mScrollArea.cellWidth = 0;      //  todo
        mScrollArea.cellHeight = 0;     //  ^^^^        and same for corners
        ...
        mFrame.widthPx = widthRef - mBorders[LEFT].pixels - mBorders[RIGHT].pixels;
        mFrame.heightPx = heightRef - mBorders[TOP].pixels - mBorders[BOTTOM].pixels;
        //todo distract/adapt
        distractAdapt();
    ...
    private void distractAdapt() {     //  TODO    adaptation adjusting
        //int providedWidth = widthRef - mBorders[LEFT].pixels - mBorders[RIGHT].pixels;
        if (mScrollArea.columns * mScrollArea.cellWidth < mFrame.widthPx) {
            mScrollArea.cellWidth = mFrame.widthPx / mScrollArea.columns + 1;
        }
        //int providedHeight = heightRef -mBorders[TOP].pixels - mBorders[BOTTOM].pixels;
        if (mScrollArea.rows * mScrollArea.cellHeight < mFrame.heightPx) {
            mScrollArea.cellHeight = mFrame.heightPx / mScrollArea.rows + 1;
        }
    }
//--------------------------------------------------------------------------------------------------
